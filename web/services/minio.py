...
from   web.model.t_dmmx   import get_dmm_from_dm,get_backup_server,get_db_backup_server,get_db_backup_tags,get_sync_server,get_db_backup_tags_by_env_type
...
class minio_log_query(basehandler):
    ...
    def post(self):
        ...
        v_list     = query_minio_log(tagname,db_env,begin_date,end_date)
        v_json     = json.dumps(v_list)
        self.write(v_json)

class minio_log_query_detail(basehandler):
    @tornado.web.authenticated
    def post(self):
        self.set_header("Content-Type", "application/json; charset=UTF-8")
        tagname  = self.get_argument("tagname")
        backup_date = self.get_argument("backup_date")
        v_list = query_minio_log_detail(tagname,backup_date)
        v_json = json.dumps(v_list)
        self.write(v_json)
...
class minio_log_analyze(basehandler):
    ...
    def post(self):
        ...
        end_date   = self.get_argument("end_date")
        d_list     = {}
        v_list1,v_list2 = query_minio_log_analyze(db_env,db_type,tagname,begin_date,end_date)
...
class get_minio_tasks(basehandler):
    @tornado.web.authenticated
    def post(self):
        self.set_header("Content-Type", "application/json; charset=UTF-8")
        db_env  = self.get_argument("db_env")
        db_type = self.get_argument("db_type")
        d_list  = {}
        v_list  = get_db_minio_tags_by_env_type(db_env,db_type)
        d_list['data'] = v_list
        v_json  = json.dumps(d_list)
